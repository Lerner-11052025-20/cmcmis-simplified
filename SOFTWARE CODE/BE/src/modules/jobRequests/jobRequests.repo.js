...
async function insertJobRequest(conn, payload) {
  ...
  await conn.query(
    `INSERT INTO cmms_jobrequest_mst (
       ...
       JR_PROJECTID, JR_SUBSYSTEM, JR_DESIGNATION, JR_DIVISION, JR_APPROVING_AUTHORITY,
    ...
    [
      ...
      payload.division_id,
      tr(payload.approving_authority_employee_id, 7),
...
async function getJobRequestsForExport(startId, endId) {
  const [rows] = await pool.query(
    `SELECT
       jr.JR_JOBREQUESTNO                            AS jr_no,
       jr.JR_EQM_NAME                                AS equipment_name,
     ...
     LEFT JOIN cmms_section_mst sm ON sm.SM_ID = jr.JR_DIVISION
     WHERE jr.JR_JOBREQUESTNO >= ?
       AND jr.JR_JOBREQUESTNO <= ?
       AND jr.JR_CANCELLED_AT IS NULL
     ORDER BY jr.JR_JOBREQUESTNO ASC`,
