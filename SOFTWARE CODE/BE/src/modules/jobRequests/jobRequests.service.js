...
async function convertJobRequest({ jrNo, body, actor, ipAddress, userAgent }) {
  ...
  try {
    ...
    await jcRepo.insertFromJobRequest(conn, {
      ...
      job_request_received_date: body.job_request_received_date,
...
const { renderJobRequestListPdf, formatJobCategoryType, formatStatus } = require('../pdf/templates/jobRequestList');

async function prepareJobRequestPdfExport({ startId, endId, actor }) {
  const rows = await repo.getJobRequestsForExport(startId, endId);
  ...
  const items = rows.map((r) => ({
    ...
    date: r.submitted_at || r.created_at
      ? dayjs(r.submitted_at || r.created_at).format('YYYY-MM-DD')
      : '—',
    status: formatStatus(r.status),
  }));

  const rangeText = `${startId} to ${endId}`;
  const filename = `job_requests_${startId}_to_${endId}.pdf`;
  ...
  return {
    filename,
    render: (stream) => renderJobRequestListPdf({
      rows: items,
      requester,
      rangeText,
    }, stream),
