...
async function insertFromJobRequest(conn, payload) {
  ...
  await conn.query(
    ...
    [
      ...
      payload.job_request_received_date,
...
async function findByIdWithDetails(sectionJobNo) {
  const [rows] = await pool.query(
    `SELECT
       ...
       jc.JM_JOB_TYPE                   AS work_type,
       jc.JM_LANE_CODE                  AS lane_code,
       jc.job_request_received_date     AS job_request_received_date,
       ...
       jc.awaiting_from_date, jc.awaiting_restarting_date, jc.awaiting_clear_date, jc.attended_by,
       ...
       jc.repair_equipment_received_from_cal_lab_flag,
       ...
       eq_prod.PROD_NAME                AS equipment_type_name,
       eq_make.CMM_CONT_NAME            AS equipment_make,
       ...
       (
         SELECT MAX(COALESCE(jc2.cal_job_completed_date, DATE(jc2.actual_completion_date), DATE(jc2.JM_JobEndDate)))
           FROM cmms_jobcard_mst jc2
          WHERE jc2.JM_EQM_TYPE = jc.JM_EQM_TYPE
            AND jc2.JM_EQM_ID = jc.JM_EQM_ID
            AND jc2.JM_JOB_TYPE = 'CALIBRATION'
            AND jc2.JM_MVP_STATUS = 'VERIFIED_CLOSED'
       ) AS equipment_last_calibration_date,
       /* engineer */
       ...
       jr.JR_PHOENLAB                   AS submitted_by_lab_phone,
       jr.JR_PHONEROOM                  AS submitted_by_room_phone,
     ...
     LEFT JOIN cmms_product_mst    eq_prod  ON eq_prod.PROD_ID = e.EQM_INST_TYPE
     LEFT JOIN sections            eq_sec   ON eq_sec.section_id = e.EQM_SECTION_ID
     LEFT JOIN cmms_section_mst    eq_sm    ON eq_sm.SM_ID = e.EQM_DIVID
