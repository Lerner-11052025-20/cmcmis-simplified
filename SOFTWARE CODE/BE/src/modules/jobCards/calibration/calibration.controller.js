// ============================================================================
// src/modules/jobCards/calibration/calibration.controller.js
// ============================================================================

'use strict';

const service = require('./calibration.service');
...
async function listAdjustmentRows(req, res, next) {
  try {
    const items = await service.listAdjustmentRows({
      sectionJobNo: req.params.id,
      actor: readActor(req),
    });
    return res.json({ data: { items } });
...
async function postAdjustmentRow(req, res, next) {
  try {
    const data = await service.addAdjustmentRow({
      sectionJobNo: req.params.id,
      body: req.body,
      actor: writeActor(req),
    });
...
async function patchAdjustmentRow(req, res, next) {
  try {
    const rowId = parseRowId(req, res);
    if (!rowId) return null;
    const data = await service.updateAdjustmentRow({
      sectionJobNo: req.params.id,
      rowId,
      body: req.body,
      actor: writeActor(req),
...
async function deleteAdjustmentRow(req, res, next) {
  try {
    const rowId = parseRowId(req, res);
    if (!rowId) return null;
    const data = await service.deleteAdjustmentRow({
      sectionJobNo: req.params.id,
      rowId,
      actor: writeActor(req),
    });
