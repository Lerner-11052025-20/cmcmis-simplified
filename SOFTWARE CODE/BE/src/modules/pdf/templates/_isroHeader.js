...
function drawHeader(doc, { title, subtitle, compact = false } = {}) {
  ...
  if (ISRO_LOGO_PATH) {
    try {
      // Draw rectangular border box around the ISRO logo
      doc.lineWidth(0.6).strokeColor(COLORS.border);
      doc.rect(leftX, startY, sealW, sealH).stroke();
      
      // Draw ISRO logo image centered inside the box
      doc.image(ISRO_LOGO_PATH, leftX + 2, startY + 2, { 
        fit: [sealW - 4, sealH - 4], 
        align: 'center', 
        valign: 'center' 
      });
  ...
  if (SAC_LOGO_PATH) {
    try {
      // Draw rectangular border box around the SAC logo
      doc.lineWidth(0.6).strokeColor(COLORS.border);
      doc.rect(sacX, startY, sealW, sealH).stroke();
      
      // Draw SAC logo image centered inside the box
      doc.image(SAC_LOGO_PATH, sacX + 2, startY + 2, { 
        fit: [sealW - 4, sealH - 4], 
        align: 'center', 
        valign: 'center' 
      });
...
function stampPageNumbers(doc, { docId } = {}) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    const y = doc.page.height - 24; // Draw well below table bounds (A4 Landscape height is 595.28)
    const leftX  = PAGE_MARGIN_X;
    ...
    doc.text(leftStr, leftX, y, { width: rightX - leftX, align: 'left', lineBreak: false });
