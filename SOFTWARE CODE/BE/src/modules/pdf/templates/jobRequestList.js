// ============================================================================
// src/modules/pdf/templates/jobRequestList.js
// ----------------------------------------------------------------------------
// PDF — JOB REQUEST LIST REPORT (landscape multi-page)
//
// PHASE 15 — PDF Export Feature
//
// Professional, landscape report listing job requests inside the specified range.
...
function formatJobCategoryType(row = {}) {
  ...
  const type = TYPE_LABELS[row.job_type] || row.job_type || '';
...
function addNewPage(doc, columns, widths, tableW) {
  ...
  drawHeader(doc, {
    title: 'JOB REQUEST EXPORT REPORT',
...
function drawMetadataBox(doc, { requester, rangeText, stats }) {
  ...
  doc.text('JOB ID EXPORT RANGE', col2X, y + 8);
  doc.font('Helvetica-Bold').fontSize(9).fillColor(COLORS.title);
...
function drawJobRequestsTable(doc, rows) {
  const x = PAGE_MARGIN_X;
  const tableW = doc.page.width - PAGE_MARGIN_X * 2;

  // 7 columns matching the Job Request list table layout
  const columns = [
    { header: 'Job ID',             key: 'request_code',    width: 100 },
    { header: 'Equipment',          key: 'equipment_name',  width: 170 },
    { header: 'Category / Type',    key: 'job_category_type', width: 130 },
    { header: 'Division',           key: 'division_code',   width: 80 },
    { header: 'Submitted By',       key: 'submitted_by_name', width: 110 },
  ...
  if (!rows || rows.length === 0) {
    ...
    doc.text('No job request records found inside this ID range.', x + 10, doc.y + 10);
    doc.y += 24;
    return;
  }
...
function renderJobRequestListPdf({ rows, requester, rangeText }, stream) {
  ...
  // Data table
  drawJobRequestsTable(doc, rows);

  stampPageNumbers(doc, { docId: 'JOB-REQUESTS' });
  doc.end();
}

module.exports = {
  renderJobRequestListPdf,
