// ============================================================================
// src/modules/pdf/templates/jobCardList.js
// ----------------------------------------------------------------------------
// PDF — JOB CARD LIST REPORT (landscape multi-page)
//
// PHASE 15 — PDF Export Feature
//
// Professional, landscape report listing job cards inside the specified range.
...
const ODD_ROW_BG = '#EDF1F7'; // slate-grey contrast color from FE DataTable
...
function formatJobCategoryType(row = {}) {
  ...
  const type = TYPE_LABELS[row.work_type || row.job_type] || row.work_type || row.job_type || '';
...
function drawMetadataBox(doc, { requester, rangeText, stats }) {
  ...
  doc.text('JOB CARD ID EXPORT RANGE', col2X, y + 8);
  doc.font('Helvetica-Bold').fontSize(9).fillColor(COLORS.title);
...
function drawJobCardsTable(doc, rows) {
  const x = PAGE_MARGIN_X;
  const tableW = doc.page.width - PAGE_MARGIN_X * 2;

  // 8 columns matching the Job Card list layout in Landscape
  const columns = [
    { header: 'Job Card ID',         key: 'card_code',        width: 90 },
    { header: 'Job Request ID',      key: 'job_request_code', width: 90 },
    { header: 'Category / Type',    key: 'job_category_type', width: 110 },
    { header: 'Equipment',          key: 'equipment_name',   width: 140 },
    { header: 'Assigned Engineer',  key: 'assigned_engineer_name', width: 120 },
    { header: 'Status',             key: 'status',           width: 80 },
    { header: 'Start Date',         key: 'start_date',       width: 80 },
  ...
  if (!rows || rows.length === 0) {
    ...
    doc.text('No job card records found inside this ID range.', x + 10, doc.y + 10);
    doc.y += 24;
    return;
  }
...
function renderJobCardListPdf({ rows, requester, rangeText }, stream) {
  ...
  // Data table
  drawJobCardsTable(doc, rows);

  stampPageNumbers(doc, { docId: 'JOB-CARDS' });
  doc.end();
}

module.exports = {
  renderJobCardListPdf,
  formatJobCategoryType,
  formatStatus,
};
